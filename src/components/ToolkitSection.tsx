...
import { Grid, ExternalLink, X, Sliders } from "lucide-react";
...
export default function ToolkitSection({ tools, lang }: ToolkitSectionProps) {
  ...
  const limit = 15;
  const hasOverflow = tools.length > limit;
  const displayedTools = hasOverflow ? tools.slice(0, limit) : tools;

  // Filter tools inside the modal
  const filteredModalTools = tools.filter(tool => {
    const q = searchQuery.toLowerCase();
    const name = lang === "en" ? tool.nameEn : tool.nameNp;
    const cat = lang === "en" ? tool.categoryEn : tool.categoryNp;
    const desc = lang === "en" ? tool.descriptionEn : tool.descriptionNp;
    return (
